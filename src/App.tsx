...
import { Play, Tv, Music, Radio, Globe, Rocket, Newspaper, ChevronRight, ExternalLink, Settings, X, Plus, Film, Trophy, Baby, Lock, Key, Tag, Trash2, Search } from 'lucide-react';
...
export default function App() {
  ...
  }, [channels, categories, adminPhoto]);
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-6 overflow-hidden flex-1">
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-neon-pink to-neon-cyan p-[2px] animate-rotate overflow-hidden">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-4">
                    <label className="block text-[10px] font-mono text-white/40 uppercase mb-2">Admin Photo URL</label>
                    <input 
                      type="text" 
                      value={adminPhoto}
                      onChange={e => setAdminPhoto(e.target.value)}
                      placeholder="https://image-url.com/photo.jpg"
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-neon-pink transition-colors mb-4"
                    />
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-neon-pink/20 to-neon-cyan/20 flex items-center justify-center border border-white/10">
                  {getIcon(currentChannel.icon)}
                </div>
                <div>
                  <h2 className="text-xl font-bold text-white">{currentChannel.name}</h2>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs font-mono text-neon-cyan uppercase tracking-wider">{currentChannel.category}</span>
              ...
              <div className="flex items-center gap-2">
                <button className="flex-1 md:flex-none px-4 py-2 rounded-xl bg-neon-pink text-white text-sm font-semibold hover:bg-neon-pink/80 transition-colors shadow-[0_0_15px_rgba(255,0,127,0.3)]">
                  Subscribe
                </button>
                <a 
        ...
        <aside className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass rounded-2xl border border-white/10 flex flex-col h-[600px] lg:h-[calc(100vh-12rem)] sticky top-24">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {filteredChannels.length > 0 ? (
                filteredChannels.map((channel) => (
                  ...
                  >
                    <div className={`
                      w-8 h-8 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-3
                      ${currentChannel.id === channel.id ? 'bg-neon-cyan/20' : 'bg-white/5'}
                    `}>
                    ...
                    {currentChannel.id === channel.id && (
                      <motion.div 
                        ...
                        className="w-1.5 h-1.5 rounded-full bg-neon-cyan shadow-[0_0_8px_rgba(0,242,255,0.8)]"
                      />
                    )}
                  </button>
                ))
                ...
                <div className="flex flex-col items-center justify-center h-full opacity-20 gap-4">
                  ...
                  <p className="text-xs font-mono">NO CHANNELS IN THIS CATEGORY</p>
                </div>
              )}
            </div>
      ...
      {/* Footer */}
      <footer className="mt-auto glass border-t border-white/10 p-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col items-center md:items-start gap-2">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-neon-pink to-neon-cyan p-[1px]">
                <div className="w-full h-full rounded-full bg-[#0a0a0f] flex items-center justify-center">
                  <Play className="w-3 h-3 text-neon-cyan" />
                </div>
              </div>
              <span className="font-bold tracking-tighter text-white">NkTVsetup</span>
            </div>
            <p className="text-xs text-white/40">© {new Date().getFullYear()} NkTVsetup. All rights reserved.</p>
          </div>
          ...
          <div className="flex items-center gap-8 text-xs font-medium text-white/40">
            ...
            <span className="hover:text-neon-pink cursor-pointer transition-colors">Privacy Policy</span>
            <span className="hover:text-neon-pink cursor-pointer transition-colors">Terms of Service</span>
          </div>
        </div>
      </footer>
    </div>
